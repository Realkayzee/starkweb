...
export type GetStarknetIdExtendedVerifierDataParameters = Prettify<{
  /** Starknet ID, domain, or address to get the extended verifier data for. */
  idDomainOrAddr: string
  /** Field to retrieve from extended verifier data. */
  field: string
  /** Length of the extended verifier data to retrieve. */
  length: number
  /** Optional verifier contract address. */
  verifier?: string
}>

export type GetStarknetIdExtendedVerifierDataReturnType = bigint[]

export type GetStarknetIdExtendedVerifierDataErrorType = Error

export async function getStarknetIdExtendedVerifierData<
...
>(
  client: Client<Transport, TChain, TAccount>,
  {
    ...
    length,
    verifier,
  }: GetStarknetIdExtendedVerifierDataParameters,
  ...
  try {
    ...
    throw new Error(
      `Could not get user extended verifier data from starknet id: ${error}`,
    )
  }
}
