...
export type GetStarknetIdUnboundedVerifierDataParameters = Prettify<{
  /** Starknet ID, domain, or address to get the unbounded verifier data for. */
  idDomainOrAddr: string
  /** Field to retrieve from unbounded verifier data. */
  field: string
  /** Optional verifier contract address. */
  verifier?: string
}>

export type GetStarknetIdUnboundedVerifierDataReturnType = bigint[]

export type GetStarknetIdUnboundedVerifierDataErrorType = Error

export async function getStarknetIdUnboundedVerifierData<
...
>(
  client: Client<Transport, TChain, TAccount>,
  {
    ...
    field,
    verifier,
  }: GetStarknetIdUnboundedVerifierDataParameters,
  ...
  try {
    ...
    throw new Error(
      `Could not get user unbounded verifier data from starknet id: ${error}`,
    )
  }
}
